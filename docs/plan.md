...
### Part 5 — Actor (team-specific) 🔲 Team-specific

The strategy brain. Decides the next action given `ObservationState`. Each team implements its own.

```
src/actor/
  actor.py        # base interface: decide(obs: ObservationState) → str (action)
  heuristic.py    # example: Manhattan distance cop, random walk thief
```

The Actor is the only part not shared between teams. It plugs into the Agent's retry loop as the
function that produces the LLM prompt + calls the LLM.
